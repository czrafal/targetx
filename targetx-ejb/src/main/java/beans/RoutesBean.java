...
public class RoutesBean {
	...
	@EJB
	ConverterEntityToDTO entityConverter;
...
//		RouteDTO routeDTO = new RouteDTO();
		...
		Query query = entityManager.createNamedQuery(Route.GET_ROUTE_BY_DAY_AND_DRIVER);
...
//		RouteDTO routeDTO = new RouteDTO();
		Map<Driver, List<RouteFromEntityDTO>> routeMap = new HashMap<Driver, List<RouteFromEntityDTO>>();
		Query query = entityManager.createNamedQuery(Route.GET_ROUTE_BY_MONTH_AND_DRIVER);
		query.setParameter("IDSystem", new Long(2));
		query.setParameter("dateStart", new Timestamp(date.getTime()),TemporalType.DATE);
		query.setParameter("LName", '%' + search + '%');
		routeList = query.getResultList();
...
//		return routeListDTO;
	...
	public List<WorkTimeDTO> getWorkTimeByDayRangeAndDriver(Date dateStart, Date dateEnd, String name) {
		...
		Query query = entityManager.createNamedQuery(Route.GET_WORKTIME_BY_DAY_RANGE_AND_NAME);
	...
	public Map<Driver, List<RouteFromEntityDTO>> routeListDTOToRouteMap(List<RouteDTO> routeListDTO){
		Map<Driver, List<RouteFromEntityDTO>> routeMap = new HashMap<Driver, List<RouteFromEntityDTO>>();
	...
	public List<RouteDTO> getRouteByDayDetails(Date date) {
		...
		if (date != null) {
			...
			for (int i = 0; i < routeList.size(); i++) {
				...
				Driver driver = route.getDriver();
				routeDTO.setDriver(driver);
//				Period period = new Period(new DateTime(routeDTO.getRoute().getDateStart()), new DateTime(routeDTO.getRoute().getDateEnd()));
...
//				routeDTO.setTimePeriodString(PeriodFormat.getDefault().print(period));
	...
	/*
	 * Filtrowanie czasu pracy przez zakres dat
	 * Filtrowanie tras po zakresie dat
	 */
	@SuppressWarnings("unchecked")
...
/*			
			for(int i=0;i<workTimeDTOList.size();i++){
				WorkTimeDTO timeDTO = workTimeDTOList.get(i);
				for(int p=0;p<workTimeDTOList.size();p++){
					if(workTimeDTOList.get(i).getDriver() == driver && workTimeDTOList.get(i).getRoute().getIDRoutes()!=route.getIDRoutes()){
						WorkTimeDTO newDTO = new WorkTimeDTO();
						newDTO = workTimeDTO;
						newDTO.setDistanceSum(distanceSum+timeDTO.getDistanceSum());
						newDTO.setFuelEnd(fuelSum+timeDTO.getFuelEnd());
						workTimeDTORetList.add(i, newDTO);
					}else{
