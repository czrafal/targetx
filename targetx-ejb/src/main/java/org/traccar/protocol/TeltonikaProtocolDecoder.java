...
public class TeltonikaProtocolDecoder extends BaseProtocolDecoder {
    ...
    private DeviceSession parseIdentification(Channel channel, SocketAddress remoteAddress, ChannelBuffer buf) {
    	System.out.println("TeltonikaProtocolDecoder parseIdentificatin");
        int length = buf.readUnsignedShort();
        String imei = buf.toString(buf.readerIndex(), length, StandardCharsets.US_ASCII);
        System.out.println("TeltonikaProtocolDecoder parseIdentification received imei:"+imei);
        DeviceSession deviceSession = getDeviceSession(channel, remoteAddress, imei);
      
        System.out.println("TeltonikaProtocolDecoder parseIdentification chanel "+channel !=null);
        if (channel != null) {
        	System.out.println("TeltonikaProtocolDecoder parseIdentification channel is not null");
            ChannelBuffer response = ChannelBuffers.directBuffer(1);
            if (deviceSession != null) {
            	System.out.println("TeltonikaProtocolDecoder parseIdentification deviceSession is not null "+deviceSession.getDeviceId());
                response.writeByte(1);
                System.out.println(getClass().getName()+ "writeResponse 1");
            } else {
            	System.out.println("TeltonikaProtocolDecoder parseIdentification deviceSession is null");
                response.writeByte(0);
                System.out.println(getClass().getName()+ "writeResponse 2");
            }
            System.out.println(getClass().getName()+ "writeResponse:");
    ...
    private void decodeSerial(Position position, ChannelBuffer buf) {
    	System.out.println("TeltonikaProtocolDecoder decodeserial");
    ...
    private void decodeParameter(Position position, int id, ChannelBuffer buf, int length) {
    	System.out.println("TeltonikaProtocolDecoder decodeParameter id "+id);
        switch (id) {
            case 1:
            case 2:
            case 3:
    ...
    private void decodeLocation(Position position, ChannelBuffer buf, int codec) {
    	System.out.println("TeltonikaProtocolDecoder decode location");
        int globalMask = 0x0f;
...
//                    position.setNetwork(new Network(cellTower));
    ...
    private List<Position> parseData(Channel channel, SocketAddress remoteAddress, ChannelBuffer buf, int locationPacketId, String... imei) {
        ...
        System.out.println("TeltonikaProtocolDecoder.ParseData");
...
*/
        for (int i = 0; i < count; i++) {
            ...
            System.out.println("loop "+getProtocolName());
            ...
            Context.getIdentityManager().savePosition(position);
            System.out.println("parseData position deviceId:"+ deviceSession.getDeviceId()+" altitude:"+position.getAltitude() +" latitude:"+position.getLatitude() +" longitude:"+position.getLongitude() +"speed:"+position.getSpeed()+" addres:"+position.getAddress()+" devicetime:"+position.getDeviceTime().toString()+" fixTime:"+position.getFixTime().toString() );
        }
        ...
        System.out.println("TeltonikaProtocolDecoder.ParseData 2");
        return positions;
    }
    ...
    protected Object decode(Channel channel, SocketAddress remoteAddress, Object msg) throws Exception {
        ...
        System.out.println("TeltonikaProtocolDecoder decode");
        if (channel instanceof DatagramChannel) {
    ...
    private Object decodeTcp(Channel channel, SocketAddress remoteAddress, ChannelBuffer buf) throws Exception {
    	System.out.println("TeltonikaProtocolDecoder decode tcp");
        if (buf.getUnsignedShort(0) > 0) {
            parseIdentification(channel, remoteAddress, buf);
        } else {
            buf.skipBytes(4);
            System.out.println("TeltonikaProtocolDecoder decode tcp before parseData");
    ...
    private Object decodeUdp(Channel channel, SocketAddress remoteAddress, ChannelBuffer buf) throws Exception {
    	System.out.println("TeltonikaProtocolDecoder decode udp");
